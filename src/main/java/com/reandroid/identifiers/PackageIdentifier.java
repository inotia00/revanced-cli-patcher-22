...
public class PackageIdentifier extends IdentifierMap<TypeIdentifier>{
    ...
    public void writePublicXml(File file) throws IOException {
        XmlSerializers serializer = XMLFactory.newSerializer(file);
        write(serializer);
        IOUtil.close(serializer);
    }
    public void writePublicXml(OutputStream outputStream) throws IOException {
        XmlSerializers serializer = XMLFactory.newSerializer(outputStream);
        serializer.setOutput(outputStream, StandardCharsets.UTF_8.name());
        write(serializer);
    }
    public void write(XmlSerializers serializer) throws IOException {
    ...
    private void writePackageInfo(XmlSerializers serializer) throws IOException {
        String name = getName();
        if(name != null){
    ...
    private void writeTypes(XmlSerializers serializer) throws IOException {
        for(TypeIdentifier typeIdentifier : list()){
    ...
    public void loadPublicXml(File file) throws IOException, XmlPullParsersException {
        XmlPullParsers parser = XMLFactory.newPullParser(file);
        loadPublicXml(parser);
    }
    public void loadPublicXml(InputStream inputStream) throws IOException, XmlPullParsersException {
        XmlPullParsers parser = XMLFactory.newPullParser(inputStream);
        loadPublicXml(parser);
    }
    public void loadPublicXml(Reader reader) throws IOException, XmlPullParsersException {
        XmlPullParsers parser = XMLFactory.newPullParser(reader);
        loadPublicXml(parser);
    }
    public void loadPublicXml(XmlPullParsers parser) throws IOException, XmlPullParsersException {
        boolean resourcesFound = false;
        int event;
        while ((event = parser.nextToken()) != XmlPullParsers.END_DOCUMENT){
            ...
            if(!resourcesFound){
                ...
                if(!resourcesFound){
                    throw new XmlPullParsersException("Invalid public.xml, expecting first tag '"
    ...
    private void closeParser(XmlPullParsers parser){
    ...
    private void closeSerializer(XmlSerializers serializer){
    ...
    private void loadPackageInfo(XmlPullParsers parser){
    ...
    private void parseEntry(XmlPullParsers parser) throws XmlPullParsersException {
        if(!XML_TAG_PUBLIC.equals(parser.getName())){
            throw new XmlPullParsersException("Invalid tag, expecting '"
        ...
        if(typeName == null){
            throw new XmlPullParsersException("Missing attribute '"
