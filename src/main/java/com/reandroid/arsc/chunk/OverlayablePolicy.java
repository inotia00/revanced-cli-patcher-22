...
public class OverlayablePolicy extends Chunk<OverlayablePolicyHeader> implements
    ...
    public void parse(XmlPullParsers parser) throws IOException, XmlPullParsersException {
        if (parser.getEventType() != XmlPullParsers.START_TAG) {
            throw new XmlPullParsersException("Parser not in START_TAG event: " + parser.getEventType());
        }
        if (!TAG_policy.equals(parser.getName())) {
            throw new XmlPullParsersException("Expecting tag '" + TAG_policy +
                    "', but found '" + parser.getName() + "'");
        }
        setFlags(PolicyFlag.parse(parser));
        parser.next();
        XMLUtil.ensureTag(parser);
        while (parser.getEventType() != XmlPullParsers.END_TAG &&
                parser.getEventType() != XmlPullParsers.END_DOCUMENT) {
