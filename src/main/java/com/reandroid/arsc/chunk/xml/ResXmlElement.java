...
public class ResXmlElement extends ResXmlNode implements
    ...
    @Override
    public void serialize(XmlSerializers serializer, boolean decode) throws IOException {
        int count = getNamespaceCount();
        for(int i = 0; i < count; i++){
    ...
    @Override
    public void parse(XmlPullParsers parser) throws IOException, XmlPullParsersException {
        if(parser.getEventType() != XmlPullParsers.START_TAG){
            throw new XmlPullParsersException("Invalid state START_TAG != "
                    + parser.getEventType());
        }
    ...
    private void parseChildes(XmlPullParsers parser) throws IOException, XmlPullParsersException {
        ...
        while (event != XmlPullParsers.END_TAG && event != XmlPullParsers.END_DOCUMENT){
            ...
            }else if(event == XmlPullParsers.COMMENT){
                currentElement.setComment(parser.getText());
            }
            event = parser.next();
        }
    }
    private void parseNamespaces(XmlPullParsers parser) throws XmlPullParsersException {
    ...
    private void parseAttributes(XmlPullParsers parser) throws IOException {
    ...
    private static boolean getFeatureSafe(XmlSerializers serializer, String name){
        try{
    ...
    static void setIndent(XmlSerializers serializer, boolean state){
        setFeatureSafe(serializer, FEATURE_INDENT_OUTPUT, state);
    }
    private static void setFeatureSafe(XmlSerializers serializer, String name, boolean state){
        try{
