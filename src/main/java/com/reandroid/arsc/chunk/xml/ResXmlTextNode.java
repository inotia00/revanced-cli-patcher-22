...
public class ResXmlTextNode extends ResXmlNode {
    ...
    public void parse(XmlPullParsers parser) throws IOException, XmlPullParsersException {
        ...
        if(event == XmlPullParsers.ENTITY_REF){
            text = decodeEntityRef(parser.getText());
        }else if(event == XmlPullParsers.TEXT){
            text = parser.getText();
            text = XmlSanitizer.unEscapeUnQuote(text);
        }else {
            throw new XmlPullParsersException("Invalid text event: "
    ...
    static boolean isTextEvent(int event){
        return event == XmlPullParsers.TEXT
                || event == XmlPullParsers.ENTITY_REF;
    }
