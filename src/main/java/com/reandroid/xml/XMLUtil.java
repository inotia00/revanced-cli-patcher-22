...
public class XMLUtil {
    ...
    public static int ensureStartTag(XmlPullParsers parser)
            throws IOException, XmlPullParsersException {
        int event = parser.getEventType();
        while (event != XmlPullParsers.START_TAG
                && event != XmlPullParsers.END_DOCUMENT){
            event = parser.next();
        }
        return event;
    }
    public static int ensureTag(XmlPullParsers parser)
            throws IOException, XmlPullParsersException {
        int event = parser.getEventType();
        while (event != XmlPullParsers.START_TAG &&
                event != XmlPullParsers.END_TAG  &&
                event != XmlPullParsers.END_DOCUMENT){
            event = parser.next();
        }
        return event;
    ...
    public static boolean getFeatureSafe(XmlPullParsers parser, String name, boolean def){
        try {
    ...
    public static void setFeatureSafe(XmlPullParsers parser, String name, boolean state){
        try{
    ...
    public static void close(XmlSerializers serializer) {
        if(serializer != null) {
            try {
                serializer.flush();
