...
public class XMLDocument extends XMLNodeTree implements Document<XMLElement> {
    ...
    public void parse(XmlPullParsers parser) throws XmlPullParsersException, IOException {
        encoding = null;
        standalone = null;
        clear();
        ...
        }else if(event == XmlPullParsers.END_TAG || event == XmlPullParsers.START_TAG){
            parser.next();
        }else if(event == XmlPullParsers.END_DOCUMENT){
            return;
        }
        XMLUtil.ensureStartTag(parser);
    ...
    public void parseInner(XmlPullParsers parser) throws XmlPullParsersException, IOException {
        encoding = null;
        standalone = null;
        clear();
        int event = parser.getEventType();
        ...
        if(event == XmlPullParsers.END_DOCUMENT){
            return;
        }
        if(event != XmlPullParsers.START_TAG){
            throw new XmlPullParsersException("Invalid document event: " + event);
        }
        parser.next();
        parseAll(parser);
    }
    private void parseAll(XmlPullParsers parser) throws XmlPullParsersException, IOException {
        int event = parser.getEventType();
        while (event != XmlPullParsers.END_TAG && event != XmlPullParsers.END_DOCUMENT){
    ...
    @Override
    void startSerialize(XmlSerializers serializer) throws IOException {
        if(encoding == null){
            return;
        }
    ...
    @Override
    void endSerialize(XmlSerializers serializer) {
        if(encoding == null){
            return;
        }
        try {
    ...
    public static XMLDocument load(String text) throws XmlPullParsersException, IOException {
        XMLDocument document = new XMLDocument();
        document.parse(XMLFactory.newPullParser(text));
        return document;
    }
    public static XMLDocument load(InputStream inputStream) throws XmlPullParsersException, IOException {
        XMLDocument document = new XMLDocument();
        document.parse(XMLFactory.newPullParser(inputStream));
        return document;
    }
    public static XMLDocument load(File file) throws XmlPullParsersException, IOException {
        XMLDocument document = new XMLDocument();
