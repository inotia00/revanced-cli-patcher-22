...
public class AndroidManifestHelper {
            ...
            if (value.equals("com.android.dynamic.apk.fused.modules")) {
                ResXmlAttribute attribute = element.searchAttributeByResourceId(AndroidManifest.ID_value);
                    if (attribute != null) {
                        ...
                        if (attributeValue != null && attributeValue.equals("base")) {
                            return true;
                        }
                    }
            }
    ...
    public static void removeAttributeFromManifestByName(AndroidManifestBlock androidManifestBlock, String resourceName) {
    ...
    public static void removeAttributeFromManifestById(AndroidManifestBlock androidManifestBlock, int resourceId) {
        ResXmlElement manifestElement = androidManifestBlock.getManifestElement();
        if (manifestElement != null) {
    ...
    public static void removeAttributeFromManifestAndApplication(AndroidManifestBlock androidManifestBlock, int resourceId) {
        if (resourceId == 0) {
            return;
        }
        ResXmlElement manifestElement = androidManifestBlock.getManifestElement();
        if (manifestElement != null) {
            manifestElement.removeAttributesWithId(resourceId);
            ResXmlElement applicationElement = manifestElement.getElement(AndroidManifest.TAG_application);
